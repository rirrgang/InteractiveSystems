...
document.querySelector('.dayPickerContainer').addEventListener("wheel", function (event) {
    if (event.deltaY < 0) {
        let monthDays = daysInMonth();
        document.querySelectorAll('.dayPickerContainer p').forEach(element => {
            element.innerHTML = parseInt(element.innerHTML) - 1;
...
document.querySelector('.timePickerContainer div:nth-child(2)').addEventListener("wheel", function (event) {
    if (event.deltaY < 0) {
        for (let i = 0, j = visualMonths.length; i < j; i++) {
            visualMonths[i] = visualMonths[i] - 1;
            if (visualMonths[i] < 0) {
...
document.querySelector('.timePickerContainer div:nth-child(3)').addEventListener("wheel", function (event) {
    if (event.deltaY < 0) {
        document.querySelectorAll('.yearPickerContainer p').forEach(element => {
            element.innerHTML = parseInt(element.innerHTML) - 1;
            calculateDays(element);
